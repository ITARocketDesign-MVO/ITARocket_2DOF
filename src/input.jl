...
using GoogleSheets
"""
    online_input(spreadsheet_id)

Leitura dos parâmetros de voo de uma planilha do Google Sheets. 
...
O ID da planilha do google se encontra na URL entre o "/d/" e o "/edit"  

# Exemplo
`https://docs.google.com/spreadsheets/d/1TAA_Smb9670jyX4x2cwONw7SeEl74sQWG3ybIVIn5yk/edit#gid=0`
spreadsheet_id = `1TAA_Smb9670jyX4x2cwONw7SeEl74sQWG3ybIVIn5yk`


Autor: Príncipe
"""
function online_input(ID_planilha::String)
    ...
    dados = Dict(zip(geral[:, 1], geral[:, 2]))

    # Opcao de tabelas
    if dados["Rocket Cd"] == "TABELA"
        cd_rocket = replace.(get(client, cd_rocket_range).values, "," => ".")
        cd_rocket = parse.(Float64, cd_rocket)
        pop!(dados, "Rocket Cd")
    else
        cd_rocket = parse(Float64, dados["Rocket Cd"])
    end

    if dados["Thrust"] == "TABELA"
        prop_thrust = replace.(get(client, prop_thrust_range).values, "," => ".")
        prop_thrust = parse.(Float64, prop_thrust)
        pop!(dados, "Thrust")
    else
        prop_thrust = parse(Float64, dados["Thrust"])
    end

    if dados["Airbrake Cd"] == "TABELA"
        cd_airbrake = replace.(get(client, cd_airbrake_range).values, "," => ".")
        cd_airbrake = parse.(Float64, cd_airbrake)
        pop!(dados, "Airbrake Cd")
    else
        cd_airbrake = parse(Float64, dados["Airbrake Cd"])
    end

    option_airbrake = pop!(dados, "Airbrake option")
    airbrake_logic = pop!(dados, "Airbrake logic")

    # Convertendo strings para float
    f((k, v)) = k => parse(Float64, v)
    dados = Dict(Iterators.map(f, pairs(dados)))
    #criação de condições iniciais:
    X₀, rocket, env = manual_input(
        empty_mass=dados["Empty mass (kg)"],
        rocket_cd=cd_rocket,
        rocket_area=dados["Rocket Area (m2)"],
        thrust=prop_thrust,
        propellant_mass=dados["Propellant mass (kg)"],
        burn_time=dados["Burn time (s)"],
        airbrake_cd=cd_airbrake,
        airbrake_area=dados["Airbrake Area (m2)"],
        drogue_cd=dados["Drogue Cd"],
        drogue_area=dados["Drogue Area (m2)"],
        main_cd=dados["Main Cd"],
        main_area=dados["Main Area (m2)"],
        launch_angle=dados["Launch angle (graus)"],
        launch_altitude=dados["Launch altitude (m)"],
        rail_length=dados["Rail lenght (m)"],
        airbrake_option=option_airbrake,
        airbrake_opening_logic=(t, X, rocket, env) -> (X.y >= -7.9528299946541585 * X.vy + 3403.5517928697045),
        nozzle_area=dados["Nozzle Area (m)"]
    )
    return X₀, rocket, env
end
